...
/* LANGUAGE LAYER
   todo: separate this in to a file
 */
...
// todo: put this somewhere else (where?)
...
typedef struct _app_controller_input {
    ...
    union {
        ...
        struct {
            app_button_state MouseLeft;
            app_button_state MouseRight;
            app_button_state MouseMiddle;
            app_button_state MouseEtc1;
            app_button_state MouseEtc2;
        }
    }
...
#define APP_UPDATE(Name)                       \
    void Name(thread_context *Thread, app_memory *Memory, \
              app_input *Input)
typedef APP_UPDATE(app_update_and_render);
