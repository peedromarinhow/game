...
external APP_INIT(Init) {
 ...
 // State->UiStyle.ClickedTextColor = (colorb){0xFFAA00FF};
    ...
    // for (u32 BufferIndex = 0; BufferIndex < ArrayCount(State->CommandContext.Buffers); BufferIndex++) {
    //     State->CommandContext.Buffers[BufferIndex] = CreateBuffer(8, "a.c");
    // }
...
external APP_UPDATE(Update) {
    ...
    // r32 x = 0;
    // for (u32 BufferIndex = 0; BufferIndex < ArrayCount(State->CommandContext.Buffers); BufferIndex++) {
    //     UiAddButton(State->Renderer, &State->UiContext, &State->UiStyle, rv2_(x, 0), State->CommandContext.Buffers[BufferIndex]->Filename);
    //     x += 120;
    // }
    ...
    // c8 **AllFilenames = GlobalPlatformApi.GetAllFilenamesFromDir(p->ExecutablePath);
